...
namespace vdb_mapping_ros2_detail
{
template <typename PointCloudT>
void removePointsWithinRadiusOfOrigin(const typename PointCloudT::Ptr& cloud, double radius_m)
{
  if (radius_m <= 0.0 || !cloud || cloud->points.empty())
  {
    return;
  }
  ...
  : Node("vdb_mapping_ros2", options)
{
  ...
  if (m_apply_raw_sensor_data)
  {
    ...
    for (auto& source_id : source_ids)
    {
      ...
      this->declare_parameter<double>(source_id + ".min_sensor_range", 0.0);
      this->get_parameter(source_id + ".min_sensor_range", sensor_source.min_sensor_range);
  ...
  const std::shared_ptr<sensor_msgs::msg::PointCloud2> cloud_msg, const SensorSource& sensor_source)
{
  ...
  // Range from sensor in the cloud's frame (before map transform) — removes self-hits / prop noise.
  if (sensor_source.min_sensor_range > 0.0)
  {
    vdb_mapping_ros2_detail::removePointsWithinRadiusOfOrigin<typename VDBMappingT::PointCloudT>(
      cloud, sensor_source.min_sensor_range);
  }
