...
|---|---|---|
| `vehicle_status` **undecodable on the laptop** (30 Hz on `hz`, echo silent) | **ROOT CAUSE CONFIRMED 2026-08-11:** version mismatch. Drone runs `voxl-px4 1.14.0-2.0.142` (PX4 v1.14 wire format); the workspace's vendored px4_msgs (`src/local/controls/px4_msgs`) is the **v1.15+** layout (`ARMING_STATE_DISARMED = 1` two-state enum; v1.14 used a six-state enum) with different VehicleStatus fields → deserialization fails silently. `vehicle_odometry` is unchanged between versions, hence it echoes fine. Fix options: **(A)** upgrade the drone's voxl-px4 to the 1.15 line (ModalAI SDK ≥ 1.7) — matches the container, but re-do backups + EKF2 params after; **(B)** swap the vendored px4_msgs to PX4 `release/1.14` definitions + `bws` rebuild — `px4_interface`'s only VehicleStatus dependency (`ARMING_STATE_ARMED == 2`) is identical in v1.14, but audit the other bridged messages for layout diffs before trusting it. Decide before M6. | **M6** — `px4_interface` reads arming state from this message; also un-audited risk on command messages (e.g. `OffboardControlMode` gained trailing fields in 1.15) |
